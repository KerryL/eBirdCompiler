...
echo \#include \"plotterApp.h\">> $outputPath$outputFile
echo>> $outputPath$outputFile
echo const wxString EBirdCompilerApp::versionString = _T\(\"$gitTag\"\)\;>> $outputPath$outputFile
echo const wxString EBirdCompilerApp::gitHash = _T\(\"$gitHash\"\)\;>> $outputPath$outputFile
